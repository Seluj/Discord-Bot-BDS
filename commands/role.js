...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let membersList;
    let nb_total = 0, nb_changer = 0, nb_coti = 0, nb_non_coti = 0, nb_exception = 0, nb_esta = 0;
      ...
      interaction.guild.members.fetch()
        .then((members) => {
          ...
          for (let i = 0; i < membersList.length; i++) {
            nb_total++;
            if (membersList[i].user.bot || checkRole(membersList[i], Membre_du_Bureau) || checkRole(membersList[i], Bureau_Restreints) || checkRole(membersList[i], ESTA)) {
              nb_exception++;
